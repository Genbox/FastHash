...
public static class FarmHash64Unsafe
{
    ...
    private static unsafe ulong HashLen33to64Na(byte* data, uint length)
    {
        ulong mul = K2 + (length * 2);
        ulong a = Read64(data) * K2;
        ulong b = Read64(data + 8);
    ...
    private static unsafe ulong Hash64NaWithSeed(byte* s, uint len, ulong seed) => Hash64NaWithSeeds(s, len, K2, seed);

    private static unsafe ulong Hash64NaWithSeeds(byte* s, uint len, ulong seed0, ulong seed1) => HashLen16(Hash64Na(s, len) - seed0, seed1);
